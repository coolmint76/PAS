...
class BonePorosityAnalyzer:
    ...
    def __init__(self, 
                 te1: float = 0.05,  # First echo time in ms
                 te2: float = 2.0,   # Second echo time in ms
        ...
        # Add default voxel dimensions
        self.voxel_dims = (1.0, 1.0, 1.0)  # Default 1mm isotropic
    ...
    def calculate_porosity_index(self, 
                                echo1: np.ndarray, 
                                echo2: np.ndarray,
                                mask: Optional[np.ndarray] = None) -> np.ndarray:
        """
        Calculate the Porosity Index (PI) from dual-echo UTE data.
        ...
        -----------
        echo1 : np.ndarray
            First echo signal intensity
        echo2 : np.ndarray
            Second echo signal intensity
        mask : np.ndarray, optional
            Binary mask for cortical bone region
            
        Returns:
        --------
        pi : np.ndarray
            Porosity Index map (percentage)
        """
        # Avoid division by zero
        with np.errstate(divide='ignore', invalid='ignore'):
            pi = (echo2 / echo1) * 100.0
    ...
    def calculate_regional_statistics(self,
        ...
        if max_dist == 0:
            ...
            stats_list = [{
                ...
                'volume_mm3': mask.sum() * np.prod(self.voxel_dims)
            }]
    ...
    # Add all other methods here with proper indentation...
    # They should all be indented at the same level as the methods above
...
def demonstrate_multi_sequence_analysis():
    ...
    try:
        ...
        # Don't use DataFrame.to_string() - it can hang with certain pandas versions
        # Instead, iterate through rows manually:
